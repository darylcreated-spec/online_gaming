...
export async function GET(request: Request) {
  try {
    ...
    if (numberFilter) {
      ...
      if (!isNaN(num)) {
        if (whereClause) {
          whereClause += " AND winning_number = ?";
        } else {
          whereClause = "WHERE winning_number = ?";
        }
        args.push(num);
      }
    }
    ...
    // 1. Get total count
    const countSql = `SELECT COUNT(*) as count FROM playwhe_draws ${whereClause}`;
    ...
    console.error("[API /api/playwhe/draws] Error:", error);
    return NextResponse.json(
