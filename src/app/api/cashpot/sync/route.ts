import { syncCashPot } from "@/lib/scraper";
...
async function runSync(full: boolean, year?: number, authHeader?: string | null, secretParam?: string | null, fullSecretParam?: string | null) {
  ...
  console.log(`[API /api/cashpot/sync] Triggering Cash Pot sync (full=${full}, year=${year})...`);
  const result = await syncCashPot(full, year);
...
export async function POST(request: Request) {
  try {
    ...
    console.error("[API /api/cashpot/sync] Error:", error);
