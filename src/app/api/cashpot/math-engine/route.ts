  ...
  try {
    ...
    if (!rawDraws || rawDraws.length < 15) {
      return NextResponse.json(
        { success: false, error: "Insufficient historical draws for Cash Pot in database." },
        { status: 400 }
      );
    }
    ...
    const drawsChronological: MultiBallDrawRecord[] = rawDraws.map(d => ({
      ...
      num4: Number(d.num4),
      num5: Number(d.num5),
      powerball: Number(d.multiplier || 1) // reused for multiplier
    }));
    ...
    console.error("[API /api/cashpot/math-engine] Error:", error);
    return NextResponse.json(
      { success: false, error: error.message || "An error occurred in Cash Pot Math Engine" },
      { status: 500 }
    );
  }
}
