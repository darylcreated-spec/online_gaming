...
import { runMEVEngine } from "@/lib/mev_engine";
  ...
  try {
    ...
    if (!draws || draws.length < 10) {
      return NextResponse.json(
        { success: false, error: "Insufficient historical data for MEV analysis" },
        { status: 400 }
      );
    }

    // Run the Maximum Expected Value Engine
    const result = runMEVEngine(draws, ticketCount, candidates);
